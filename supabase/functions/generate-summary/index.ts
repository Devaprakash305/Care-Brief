...
Never invent medication names, dosages, appointments, diagnoses, or warning thresholds. If the note does not contain a value, use an empty list or explain that it was not provided. Translate every human-readable value in the JSON into ${input.language} only, including the condition, headlineSummary, medication names, medication frequency/purpose/instructions, diet and care bullets, follow-up fields, warning bullets, verification claims/source quotes/explanations, and any fallback text. Render medicine names in the requested language script using a clinically recognizable transliteration; preserve dosage units, numbers, and clinical abbreviations. For example, translate phrases such as "twice a day", "pain relief", "Take as directed", and "Avoid junk foods"; do not leave ordinary human-readable text in English. ${scriptRule} Keep JSON property names and status enum values unchanged. Do not mix in English or any other language. Apply a ${input.literacyLevel} reading level. Include only the requested sections: ${JSON.stringify(input.sections)}.
